...
class pvpframe extends JFrame implements ActionListener,MouseListener,Runnable
{
	...
	{
		int number = new Random().nextInt(32);
		int x=0,y=0,i;
		if(thread==null)
		{
		...
		if(e.getSource().equals(chessBoard))
		{
			if(flag==2&&chessPieces[respose].getName().charAt(1)=='2')
			{
				...
				{
					text.setText("red,can't play this");
					flag=2;
				}
				else
				{
				...
				user.outputinformation("move","respose e.getX() e.getY()");
			...
			else if(flag==1&&chessPieces[respose].getName().charAt(1)=='1')
			{
				...
				if(x==chessPieces[respose].getX()&&y==chessPieces[respose].getY())
				{
					text.setText("black,can't play this");
					flag=1;
				}
				else
				{
	...
	{		
		...
		{
			if(user.returnCommand=="start 1")
			{
				flag=1;
			}
			else if(user.returnCommand=="start 2")
			{
				flag=2;
			}
			chessPieces[0].setBounds(24, 56, 55, 55);
		...
		else if(event.getSource().equals(enter))
		{
			...
			str=metext.getText();
			metext.setText("");
			chattext.append("me:"+str);
			user.outputinformation("send",str);
		}
	}
